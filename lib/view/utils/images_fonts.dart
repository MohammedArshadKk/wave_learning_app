class AppImages {
  static String signInImage = 'assets/images/sign_in_image.png';
  static String signUpImage = 'assets/images/sign_up_image.png';
  static String googleIcon = 'assets/images/google.png';
  static String welcomeImage =
      'assets/images/undraw_Developer_activity_re_39tg.png';
  static String verifyImage = 'assets/images/undraw_Authentication_re_svpt.png';
  static String forgotPasswordImage =
      'assets/images/undraw_Forgot_password_re_hxwm.png';
  static String addChannelIconimage =
      'assets/images/add_channel_icon_image.png';
  static String uploadVideo = 'assets/images/video_upload.png';
  static String noDataImage = 'assets/images/undraw_Empty_re_opql.png';
  static String playlistImage = 'assets/images/playlist.png';
}

class Fonts {
  static String loginScreenFont = 'PlusJakartaSans';
  static String primaryText = 'YouTubeSans';
  static String spalshText = 'Horizon';
  static String chatBotText = 'Rubik';
  static String darkBlack = 'YouTubeSansDarkBlack';
}
